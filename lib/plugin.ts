...
const conventionalCommitConfigs: Record<
	string,
...
function renderMarkdown({ fileLimit = 50 }: { fileLimit?: number }) {
	...
	for (const {
		...
		const commitType = getConventionalCommitType(
			conventionalCommitTypes,
			message
		);
		if (commitType && message.includes(":")) {
			const [, scope, messageTitle] = /\w+(?:\((.+)?\))?:(.+)/.exec(
				message
			) as RegExpExecArray;
			const description = scope
				? `${scope}: ${messageTitle.trim()}`
				: messageTitle.trim();
...
${Object.entries(changesByType)
	.map(([changeType, messages]) =>
		renderCommitGroupMarkdown(changeType, [...new Set(messages)])
	)
	.join("\n\n")}
